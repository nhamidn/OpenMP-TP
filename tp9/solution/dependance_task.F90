...
!$ use OMP_LIB
  ...
  !$ integer                     :: nb_threads, ti, nb_taches_i, tj, nb_taches_j, rang
  integer                        :: iDeb, iFin, jDeb, jFin
  !$ integer, dimension(:), allocatable :: dep_i, dep_j

  ! Affichage du nombre de taches actives
  !$OMP PARALLEL
  !$ nb_threads = OMP_GET_NUM_THREADS()
  !$OMP END PARALLEL
  !$ print '(//,3X,"Execution dependance avec taches en parallele avec ",i2," threads")',nb_threads
  ...
  !$OMP PARALLEL PRIVATE(iDeb, iFin, jDeb, jFin)
  !$OMP SINGLE
  !$ nb_taches_i = nb_threads
  !$ nb_taches_j = 120
  !$ allocate(dep_i(1:nb_taches_i))
  !$ allocate(dep_j(1:nb_taches_j))

  !$ do ti = 1 , nb_taches_i
  !$    iDeb = 2+((ti-1)*(nx-1))/nb_taches_i
  !$    iFin = 1+(ti*(nx-1))/nb_taches_i
  !$    do tj = 1 , nb_taches_j
  !$       jDeb = 2+((tj-1)*(ny-1))/nb_taches_j
  !$       jFin = 1+(tj*(ny-1))/nb_taches_j
           !$OMP TASK DEPEND(INOUT:dep_i(ti),dep_j(tj))
